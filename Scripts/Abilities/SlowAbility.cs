...
public partial class SlowAbility : Ability
{
    ...
    [Export] public bool EnableLegacyAiSlowFallback = true;
    ...
    protected override bool Execute(Node3D caster, HealthComponent target)
    {
        ...
        if (unit is IStatusEffectReceiver statusReceiver)
        {
            statusReceiver.ApplySlow(rankSlow, Duration);
        }
        else if (EnableLegacyAiSlowFallback)
        {
            // AI migration is deliberately outside this phase.
